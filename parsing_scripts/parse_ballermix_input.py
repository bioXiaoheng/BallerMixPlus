...
def _get_ids_from_vcf_header(header, pop_list):
    ...
    print(
        f'Data from {len(ID_list)} samples will be counted.')  # The total number of samples in this vcf is {len(header[9:])}.
    ...
    while l != '':
        ...
        if l[3] in SNP and l[4] in SNP and "PASS" in l[6]:
            ...
            # otherwise, read another line
                ...
                while pos > r_pos:
                    ...
                    if rec_l == '':  # presumably end-of-file
                        print(
                            f'End-of-file for the recombination map. Assume uniform rate of {rec_rate} after {last_r_pos}.')
            ...
            if 0 < x < n:
                ...
                raise ValueError(f'Invalid x: {x}. pos={pos}')
        # read next line
        l = vcf.readline()
...
def _load_alignment(axtfile, chrom):
    ...
    while l != '':
        l = l.strip().split(" ")
        # print(l)
        # while not good_chr.search(l[1]) or not good_chr.search(l[4]):
        # skip the next 4 lines if it's not for the current chr or the matching sequence is not from the legit assembly
        while l[1] not in {str(chrom), 'chr' + str(chrom)}:  # or not good_chr.match(l[4])
        ...
        ##      if lone_atcg.match(primary_seq[i]):primary_seq[i]
        for i, base in enumerate(primary_seq):
            if base in SNP:
                pos += 1
                ...
                nonSNPs.append(base)
                # raise ValueError(f'primary_seq len={len(primary_seq)}, i={i}, base={base}.')
    ...
    # out of curiosity
    print(f'axt_lib takes memory {sys.getsizeof(axt_lib)}; '
          f'aligned_pos list takes memory of {sys.getsizeof(aligned_positions)}.')
    print('non-SNP mappings:', set(nonSNPs))
    print(len(aligned_positions), 'mapped positions recorded')
    assert len(aligned_positions) == len(axt_lib)
    # return lib
    return axt_lib, aligned_positions


def parse_axt_and_vcf(ch, axtfile, vcffile, rec_rate, outfile, pop_list, ploidy: int):
    ...
    # sanity check
    assert len(axt_lib) > 0 and len(aligned_pos) > 0, aligned_pos
    ...
    while l != '':
        ...
        while axt_pos < pos:
            ...
            if axt_ref != axt_anc:
                ...
                assert (axt_ref == axt_anc) and (axt_ref in SNP) and (axt_anc in SNP), \
                    f'axt_ref: {axt_ref}, axt_anc: {axt_anc}'
            axt_pos_i += 1
            try:
                axt_pos = aligned_pos[axt_pos_i]
            except IndexError:
        ...
        if axt_eof:
            assert axt_pos_i == len(aligned_pos)
            break
        ## Now axt_pos >= pos
        # only process bi-allelic SNPs
        if l[3] in SNP and l[4] in SNP and "PASS" in l[6]:
            ...
            elif len(set([axt_lib[pos][0], axt_lib[pos][1], l[3], l[4]])) > 2:
                print(f'{ch}, {pos}: not bi-allelic.'
                      f'\taxt-ref/anc: {axt_lib[pos]}; in vcf-ref/alt: {l[3]}/{l[4]}')
                # read on
                l = vcf.readline()
                if l == '':
                    break
            ...
            for i in individual_ids:
                ...
                # continue
                    ...
                    try:
                        ...
                        print('GT, count, e =', GT, count, e)
                        raise ValueError("This script only applies to diploid and haploid data.")
                        # sys.exit(1)
                    x += sum(count)
                    n += len(count)
            if flip:
...
def main():
    ...
    # file paths
    parser.add_argument('--vcf', dest='vcffile',
                        help='Path and name of the vcf file. Format can be either .vcf or .vcf.gz.\n', required=True)
    ...
    parser.add_argument('--ID_list', dest='pop_list', default=None,
                        help='Path and name to the file containing list of sample IDs (identical to their column names in vcf) to be counted, separated by comma. If not provided, all samples in the vcf will be counted.\n')
    parser.add_argument('--axt', dest='axtfile', default=None,
                        help='Path and name of the sequence alignment (in .axt or .axt.gz) for calling substitution and polarizing the allele frequency. If not provided, then the output will only be applicable to B_0maf.')
    parser.add_argument('--rec_rate', dest='rec_rate', type=float, default=1e-6,
                        help='Recombination rate in cM/nt. Default value is 1e-6 cM/nt.')
    parser.add_argument('--rec_map', dest='rec_map', default=None,
                        help='Path and name of the recombination map (hapmap format) of the same sequence. If not provided, a uniform recombination rate will be applied with a default rate of 1e-6 cM/nt. Use \"--rec_rate\" to specify another rate.')
    parser.add_argument('--hap', dest='hap', action='store_true', default=False,
                        help='Indicate that the organism in vcf is a haploid. Not necessary unless substitutions need to be called.')
    ...
    if opt.rec_map is not None:
        if opt.axtfile is not None:
            ...
            print(time.ctime(),
                  f'Parsing BalLeRMix input for B_2 with genetic positions matching the recombination map. Assume genomic regions not covered by the map to recombine at {opt.rec_rate} cM/nt.')
            parse_axt_and_vcf_w_recmap(opt.ch, opt.axtfile, opt.vcffile, opt.rec_map, opt.rec_rate, opt.outfile,
                                       opt.pop_list, ploidy)
        else:
            print(time.ctime(),
                  f'Parsing BalLeRMix input for B_0maf with genetic positions matching the recombination map. Assume genomic region not covered by the map to recombine at {opt.rec_rate} cM/nt.')
        ...
        if opt.axtfile is not None:
            ...
            ploidy = int(opt.hap) + (1 - opt.hap) * 2
            print(time.ctime(),
                  f'Parsing BalLeRMix input for B_2 with a uniform recombination rate of {opt.rec_rate} cM/nt...')
            parse_axt_and_vcf(opt.ch, opt.axtfile, opt.vcffile, opt.rec_rate, opt.outfile, opt.pop_list, ploidy)
        else:
            print(time.ctime(),
                  f'Parsing BalLeRMix input for B_0maf with a uniform recombination rate of {opt.rec_rate} cM/nt...')
